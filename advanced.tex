...
\begin{frame}{2025年物性若手夏の学校やります!!!}
    ...
    \textbf{\red{$\downarrow$個人協賛応募フォーム$\downarrow$}}
    \begin{figure}
        \centering
...
\begin{frame}{toric codeのWilson line}{}
    灰色の線の上のスピンを反転させる$\sigma_j^\alpha\mapsto-\sigma_j^\alpha$.
    \begin{figure}
        \centering
...
\begin{frame}{toric codeの基底状態}{}
    ...
    \begin{figure}
        ...
        \end{tikzpicture}
    \end{figure}
    loop $l_1, l_2$にと直交するloopで$\prod_{j\in l_i}\sigma_j^z$を取ると, それぞれ固有値が異なる.
    基底状態が直交して4重縮退.

    しかしこの4つは\textbf{\alert{局所的な測定で区別できない}}.
        ...
        \begin{figure}
            ...
            \begin{tikzpicture}[xscale=3]
                \foreach\x in{0,0.1,...,1}{
                    \draw[->, >=Stealth]($(\x,0)+({-90+5*sin(\x*360)}:.5)$)--++({90-5*sin(\x*360)}:1);
                }
            \end{tikzpicture}
        \end{figure}
    \end{example}
...
\begin{frame}{縮退}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.3\linewidth]{degenerate.pdf}
    \end{figure}
\end{frame}
\begin{frame}{エンタングルメント (量子もつれ)}
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \draw(-2,.5)-|(-.8,1)--(-2,1);
            \draw(2,.5)-|(.8,1)--(2,1);
            \fill[green](-.7,.5)rectangle(.7,1);1
            % \draw(2,1)-|(1.3,1.5)--(2,1.5);
            \draw(-2,1.2)-|(-1.3,1.7)--(-2,1.7);
            \fill[green](-1.2,1.2)rectangle(-.3,1.7);
            \fill[green](-.2,1.2)rectangle(.7,1.7);
            \draw(.8,1.2)rectangle(1.7,1.7);
            \draw(2,1.2)-|(1.8,1.7)--(2,1.7);
        \end{tikzpicture}
    \end{figure}
    有限幅の変換を有限回繰り返すと, 有限幅でしか状態が混ざらない.
    \begin{itemize}
        \item エンタングルメントが無限に広がっている: long range entangled (LRE)
        \item エンタングルメントが有限幅に収まる: short range entangled (SRE)
    \end{itemize}
\end{frame}
...
\begin{frame}{対称性再訪}
    ...
    \begin{figure}
        ...
        \begin{minipage}{0.45\linewidth}
            ...
            \begin{tikzpicture}
                \draw(0,0)circle(.5);
                \fill(90:.25)circle(.05);
                \fill(210:.25)circle(.05);
                \fill(-30:.25)circle(.05);
                \draw(30:.4)circle(.05);
                \draw(150:.4)circle(.05);
                \draw(270:.4)circle(.05);
            \end{tikzpicture}
        \end{minipage}
    \end{figure}
    正三角形の対称性は
