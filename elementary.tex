...
\begin{frame}{対角化}
    ...
    \begin{example}
        固有値が$\{1,5,7\}$の固有方程式はうまく変形することで
        \begin{equation*}
            \hat{U}\mqty[\diag{1,5,7}]\hat{U}^\dag\ket
        \end{equation*}
    \end{example}
\end{frame}
...
\begin{frame}{要請3}
    \begin{requirement}
        任意の状態ベクトルは「測定値」がただ一つしか生じない状態ベクトルたちの\textbf{線型結合}で書ける
    \end{requirement}
    \begin{equation*}
        \text{測定値}=\{1, 5, 7\}
        \Longrightarrow
...
\begin{frame}{要請4}
    ...
    \end{requirement}
    状態ベクトル$\ket{\psi}$で測定値セット$\{1, 5, 7\}$のうち$5$だけが出る確率
...
\begin{frame}{物理量の表し方}
    唐突ですが、物理量を測定値セットが固有値になるような行列で表してみます。
    \begin{description}
        \item[要請1] 「物理量」が一般には複数だが限定された「測定値」を持つ
        \item[要請2] どんな「物理量」にも「測定値」がただ一つしか生じない状態ベクトルが存在する
    \end{description}
    固有方程式を使うとうまく表せる。
